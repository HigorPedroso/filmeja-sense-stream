...
export const getRecommendationsByMood = (mood: MoodType): ContentItem[] => {
  // In a real implementation, this would filter based on genre mappings or AI recommendations
...
export const getPopularMovies = (): ContentItem[] => {
  return MOCK_MOVIES.filter(item => item.media_type === 'movie');
};

// Function to get popular TV shows
export const getPopularTVShows = (): ContentItem[] => {
  return MOCK_MOVIES.filter(item => item.media_type === 'tv');
};

// Function to get content details
export const getContentDetails = (id: number): ContentDetails => {
  const item = MOCK_MOVIES.find(movie => movie.id === id);
  if (!item) {
  ...
  return {
    ...
    runtime: item.media_type === 'movie' ? 150 : undefined,
    number_of_seasons: item.media_type === 'tv' ? 4 : undefined,
    streaming_providers: [
